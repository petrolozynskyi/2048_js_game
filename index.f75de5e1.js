...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    let score = 0;
    function resetGame() {
        for(let i = 0; i < squares.length; i++)squares[i].innerHTML = 0;
        score = 0;
    ...
    function clearStyles() {
        for(let i = 0; i < squares.length; i++)squares[i].style.backgroundColor = "#afa192";
    }
    ...
    const ROW_SIZE = 4;
    function moveRight() {
        for(let i = 0; i < BOARD_SIZE * ROW_SIZE; i++)if (i % 4 === 0) {
    ...
    function moveLeft() {
        for(let i = 0; i < BOARD_SIZE * ROW_SIZE; i++)if (i % ROW_SIZE === 0) {
            const totalOne = squares[i].innerHTML;
            const totalTwo = squares[i + 1].innerHTML;
            const totalThree = squares[i + 2].innerHTML;
            const totalFour = squares[i + 3].innerHTML;
    ...
    function combineRow() {
        for(let i = 0; i < BOARD_SIZE * ROW_SIZE - 1; i++)if (squares[i].innerHTML === squares[i + 1].innerHTML) {
    ...
    function combineColumn() {
        for(let i = 0; i < BOARD_SIZE * (ROW_SIZE - 1); i++)if (squares[i].innerHTML === squares[i + BOARD_SIZE].innerHTML) {
    ...
    function checkForGameOver() {
        let zeros = 0;
        for(let i = 0; i < squares.length; i++)if (squares[i].innerHTML === "0") zeros++;
        if (zeros === 0 && !canMove()) {
    ...
    function updateColors() {
        ...
        const currentState = getGameState();
        const previousState = getGameState();
        if (!compareStates(previousState, currentState)) setTimeout(()=>{
            generate();
            addColours();
        }, 200);
    }
